...
void Phaser::process(float& left, float& right)
{
	...
	float freqL = std::clamp((float)(lfo_center.get() * std::pow(2.0f, lfo_depth * leftLFO)), 1.f, srate * 0.48f);
	float freqR = std::clamp((float)(lfo_center.get() * std::pow(2.0f, lfo_depth * rightLFO)), 1.f, srate * 0.48f);
